...
`tools/clips.csv` 已經把 223 首歌的 id 都列好了,只要填兩欄:
...
改了 `songs.js` 就要重跑一次抓取,而且**要看報告**。實際跑下來,219 首裡有 6 首出問題,幾乎都是「台灣 Apple Music 沒有那個版本」造成的:
...
→ 剩下的名額從 223 首裡隨機補滿 `questionCount`。
