...
台大流行音樂演唱社社團聯展擺攤用的猜歌遊戲。掃 QR → 取暱稱 → 十題九宮格搶答 → 拿稱號。
...
| 結算、五級稱號 | 完成 |
| 音源 | Apple 官方試聽,執行時串流 |
| 排行榜 | 完成,已接上 Supabase(見下面「排行榜」一節) |
| 本地音檔 | **還沒放**,`audio/` 是空的 |
...
要清測試資料或不當暱稱,用 `staff.html`(見下面「staff.html」一節);也可以到 Supabase 後台的 Table Editor 手動刪。
...
**排行榜什麼時候讀**:只有被打開的那一下,以及切回這個分頁時(畫面還停在排行榜的話)。
沒有定時刷新。以前是開著就每 15 秒重讀一次 —— 那是給攤位想的(一支手機擺在桌上當公開榜,
要自己會動),放到網路上就變成純粹的浪費:一個人把排行榜擺著不管一小時就是 240 次
沒有人在看的讀取,切到背景照樣刷。想看新的就退出去再進來,那本來就是一個動作。
...
- **整台下架**:同一支手機的每一場次一起下架。只下架一筆的話,
  那個人第二高的成績會遞補上榜,看起來像沒下架成功 —— 要讓一個人整個從榜上消失就按這個
  (那支手機有兩筆以上紀錄時才會出現)
- **一鍵下架**(工具列):把「目前這個篩選 / 搜尋看到的全部」一次連人整台下架,
  要按兩次才執行。活動前清測試資料最快。在「已下架」分頁時它會變成**一鍵復原**,
  而且只復原列出來的那幾筆 —— 誰是被單獨下架的,不該由程式替工作人員決定
- **永久刪除**:要按兩次才執行,救不回來
...
staff.html                排行榜管理台(工作人員登入後可下架 / 一鍵下架 / 刪除)
