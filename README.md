...
- **闖關模式** —— 六關,每關十題、每題 10 秒。過關看分數(4,000 → 9,000),
...
六關,每關十題、每題 **10 秒**(競速模式是 12 秒)、滿分 10,000,分數每關獨立算。
...
難度只靠兩件事疊上去:**曲庫變大**(而且越後面越冷門)和**門檻變高**。秒數固定 10 秒 ——
每關都一樣,玩家才感覺得到「是題目變難了」,不是「我被偷偷加速了」。

解鎖獨佔一整屏(`screen-stage`)。夾在兩關中間彈個 toast 就過去的話,
玩家根本不會發現曲庫變大了 —— 而那是這個模式唯一的獎勵。
